...
# File Configs
output_file = "../../../fuzz_outputs/CPP/weekly_contest_434_p4/outputs"  # Output file to store test cases and results
cpp_folder = "../src"  # Folder containing the C++ source code
cpp_file = "weekly_contest_434_p4.cpp"  # C++ source file name
executable_name = "solution"  # Executable name

# TODO: Generate a single test case
def generate_test_input():
    random.seed(time.time())
    # 1 <= words.length <= 256, words[i].length == 2.
    # All strings in words will altogether be composed of no more than 16 unique lowercase letters. All strings in words are unique.
    n = random.randint(1, 256)
    words = []
    lowercase_letters = string.ascii_lowercase
    unique_letters = random.choices(lowercase_letters, k=16)
    for _ in range(n):
        word = random.choices(unique_letters, k=2)
        while word in words:
            word = random.choices(unique_letters, k=2)
        words.append(word)
    return n, words
