...
max_pizzas = 200000  # Maximum number of pizzas
max_pizza_slices = 100000  # Maximum number of slices in a pizza

# File Configs
output_file = "../../../fuzz_outputs/CPP/weekly_contest_437_p2/outputs"  # Output file to store test cases and results
cpp_folder = "../src"  # Folder containing the C++ source code
cpp_file = "weekly_contest_437_p2.cpp"  # C++ source file name
executable_name = "solution"  # Executable name

# TODO: Generate a single test case
def generate_test_input():
    random.seed(time.time())
    # 4 <= n == pizzas.length <= 2 * 10^5, 1 <= pizzas[i] <= 10^5, n is a multiple of 4.
    length = random.randint(4, 200000)
    pizzas = []
    for i in range(length):
        pizzas.append(random.randint(1, max_pizza_slices))
    return length, pizzas

# TODO: Format test_input as a string for terminal input simulation
def format_test_input(test_input):
    length, pizzas = test_input
    formatted_input = f"{length}\n"
    for pizza in pizzas:
        formatted_input += f"{pizza} "
