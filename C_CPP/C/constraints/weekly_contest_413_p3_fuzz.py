...
# File Configs
output_file = "../../../fuzz_outputs/C/weekly_contest_413_p3/outputs"  # Output file to store test cases and results
c_folder = "../src"  # Folder containing the C source code
c_file = "weekly_contest_413_p3.c"  # C source file name
executable_name = "solution"  # Executable name

# TODO: Generate a single test case
def generate_test_input():
    random.seed(time.time())
    # 1 <= grid.length, grid[i].length <= 10, 1 <= grid[i][j] <= 100
    grid = []
    grid_size = random.randint(1, 10)
    grid_col_size = random.randint(1, 10)
    for i in range(grid_size):
        grid.append([random.randint(1, 100) for j in range(grid_col_size)])
    return grid
...
def format_test_input(test_input):
    grid = test_input
    gridSize = len(grid)
    gridColSize = [len(row) for row in grid]
    formatted_input = f"{gridSize}\n"
    for ColSize in gridColSize:
        formatted_input += f"{ColSize} "
